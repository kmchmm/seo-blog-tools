      ...
      const pollInterval = setInterval(async () => {
        try {
          const statusResponse = await axios.get(
            `http://localhost:8011/insight-status/${item.id}`
          );
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const response = await axios.get('http://localhost:8011/insights');
        const reversed = response.data.reverse();
        const nonArchived = reversed.filter(
          (item: Insight) => item.status !== 'archived'
        );
        setInsights(nonArchived);
        setFilteredInsights(nonArchived);
        ...
        const stillPending = nonArchived.some(
          (item: Insight) =>
            item.moz_status === 'pending' || item.wincher_status === 'pending'
        );
        ...
        <div className="flex items-center justify-center relative ">
          <form onSubmit={handleSubmit} className="flex items-center w-full gap-4">
            <div className="relative w-full">
              ...
              />
              {urlError && (
                <p className="text-red-500 text-sm absolute left-0 right-0 bottom-[-20px]">
                  {urlError}
                </p> // Display error below the URL input
              )}
            </div>
            ...
            <div className="relative w-full">
              ...
              {keywordsError && (
                <p className="text-red-500 text-sm absolute left-0 right-0 bottom-[-20px]">
                  {keywordsError}
                </p> // Display error below the Keywords input
              )}
            </div>
